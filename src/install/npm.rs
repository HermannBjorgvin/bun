...
pub mod package_manifest {
    ...
    impl Serializer {
        ...
        pub(crate) fn save(
            ...
            // The temporary directory is shared by every bun process on the
            // machine, so the name must be unique per writer: concurrent
            // installs that save the same package within the same millisecond
            // used to open one file, and the rename of one moved the other's
            // bytes into its cache.
            let mut tmp_path_buf = [0u8; 64];
            let tmp_path =
                FileSystem::tmpname(b"npm", &mut tmp_path_buf, bun_core::fast_random())?;
