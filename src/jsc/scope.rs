...
use crate::{
    ArrayBuffer, CallFrame, JSGlobalObject, JSPromise, JSString, JSStringView, JSType, JSValue,
    JsClass, JsError, JsResult, Strong,
};
...
impl<'s> Scope<'s> {
    ...
    /// Migration escape hatch, like [`Self::unscoped_global`]: the VM (event
    /// loop, module loader, `.global()`) can run user JS behind this shared
    /// borrow, so nothing done through it is covered by the scope's
    /// guarantees. Pinned by the same per-file ratchet.
    #[inline(always)]
    pub fn unscoped_bun_vm(&self) -> &'static crate::virtual_machine::VirtualMachine {
        self.global.bun_vm()
    }
    ...
    /// JSString that takes over `s`'s ref (zero-copy for WTF-backed strings).
    #[inline(always)]
    pub fn transfer_string(&self, s: bun_core::String) -> JsResult<Local<'s>> {
        Ok(self.local(crate::bun_string_jsc::into_js(s, self.global)?))
    }
...
impl<'s> Local<'s> {
    ...
    /// is pinned by `test/internal/source-lints/scope-escapes.test.ts` alongside
    ...
    /// JSType-byte check; does not look through Proxies (unlike
    ...
    /// Downcast to a `.classes.ts` payload. Pure FFI type check.
    ///
    /// The payload (`m_ctx`) is a separate allocation freed by the
    /// wrapper's GC finalizer — the borrow is confined to the scope, but the
    /// scope does *not* root the wrapper. This is sound for values the JS
    /// call frame roots for the whole host call (`this`, callframe
    /// arguments), which is what host functions downcast in practice. Do
    /// not hold the payload of a freshly created, otherwise-unreferenced
    /// wrapper across an operation that can collect it; same contract as
    /// the raw [`JSValue::as_class_ref`].
    ///
    /// ```compile_fail
    ...
    /// See [`Self::to_int32`] — saturating `[0, u32::MAX]` clamp (numbers and
    /// BigInts only).
    #[inline(always)]
    ...
    /// String coercion into a borrowed view of the characters; may run a
    /// String subclass's `toString`. The guard keeps the JSString cell alive
    /// for as long as it is held, so it can be held across further JS.
    #[inline(always)]
    pub fn to_js_string_view(self, scope: &mut Scope<'s>) -> JsResult<JSStringView<'s>> {
        self.raw.to_js_string_view(scope.global)
    }
    ...
    /// [`Self::get`] filtered to non-nullish, non-empty-string values (`0`,
    /// `false`, `NaN` pass through); may run getters and Proxy traps.
    #[inline(always)]
    pub fn get_truthy(
        self,
        scope: &mut Scope<'s>,
...
impl CallFrame {
    ...
    /// The passed arguments as a branded slice — the zero-cost counterpart of
    /// [`Self::arguments`] (no copy, no `undefined` fill; length is the
    /// actual argument count).
    #[inline(always)]
    pub fn scoped_args<'a, 's>(&'a self, _scope: &Scope<'s>) -> &'a [Local<'s>] {
        let args = self.arguments();
        // SAFETY: `Local<'s>` is `repr(transparent)` over `JSValue`.
        unsafe { core::slice::from_raw_parts(args.as_ptr().cast::<Local<'s>>(), args.len()) }
    }
    ...
    pub fn scoped_arguments<'s, const MAX: usize>(
        ...
        let mut ptr = [scope.undefined(); MAX];
        // `MAX` is const, so this loop fully unrolls into per-slot selects —
        // keeping the branded read as cheap as raw slice indexing on the
        // host-call path (no memcpy, no `array::map`).
        for i in 0..MAX {
            if i < n {
                // SAFETY: `i < n <= slice.len()`.
                ptr[i] = scope.local(unsafe { *slice.get_unchecked(i) });
            }
        }
        LocalArguments { ptr, len: n }
    }
}
