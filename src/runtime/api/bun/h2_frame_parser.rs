...
/// ref (an inbound dispatch, a write that re-enters JS). Live guards are counted in
...
struct Keepalive<'a> {
    ...
    /// Dropped after the count is decremented: this release can be the last one.
    _ref: bun_ptr::ScopedRef<H2FrameParser>,
}

impl Drop for Keepalive<'_> {
    fn drop(&mut self) {
        let live = &self.parser.native_keepalives;
        debug_assert!(live.get() > 0);
        live.set(live.get() - 1);
    }
}
...
impl H2FrameParser {
    ...
    fn keepalive(&self) -> Keepalive<'_> {
        let guard = self.this_ptr().ref_guard();
...
impl H2FrameParser {
    ...
    fn release_refs_stranded_by_exit(&self) {
        ...
        self.unregister_auto_flush();
        // Each stranded `Keepalive` guard owns a ref it will never release (its frame cannot
        // resume). Release them here, under a ref of our own so the loop cannot free `self`.
        let stranded = self.native_keepalives.replace(0);
        if stranded > 0 {
            let this = self.new_ref();
            for _ in 0..stranded {
                this.deref();
            }
            this.deref();
