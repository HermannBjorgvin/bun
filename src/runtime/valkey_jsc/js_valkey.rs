...
impl RefCountedTimer {
    ...
    fn take_fire_ref(&self) -> Option<ScopedRef<JSValkeyClient>> {
        self.event_loop_timer
            .with_mut(|t| t.state = Timer::State::FIRED);
        self.held.take().map(ScopedRef::from)
    }
}
    ...
    pub(crate) fn create_no_js_no_pubsub(
        ...
        let parsed_url: OwnedUrl = 'get_url: {
            ...
            if strings::contains(url_byte_slice, b"://") {
                break 'get_url match OwnedUrl::from_utf8(url_byte_slice) {
                    Some(u) => u,
                    None => {
            ...
            match OwnedUrl::from_utf8(corrected_url) {
                Some(u) => u,
                None => {
                    return Err(
                        global_object.throw_invalid_arguments(format_args!("Invalid URL format"))
...
impl<const SSL: bool> SocketHandler<SSL> {
    ...
    pub fn on_close(
        ...
        // holds the client.
        let _socket_ref = this.socket_ref.take().map(ScopedRef::from);
...
impl bun_event_loop::ManagedTask::RunOnce for ValkeyDeferredClose {
    fn run(self) -> JsResult<()> {
        ...
        let _enqueue_ref = ScopedRef::from(self.ctx);
        match self.what {
