...
impl Interpreter {
    ...
    fn init_and_run_impl(
        ...
        if let Err(err) = shargs.parse(src, &mut [], 0) {
...
impl Interpreter {
    ...
    pub(crate) fn deinit_from_finalizer(this: Box<Self>) {
        ...
        // WTF backing. `nodes` is declared (and so dropped) before `args`, so
        // no node's AST back-reference outlives the arena.
    }
...
impl ShellArgs {
    ...
    pub(crate) fn parse(
        &mut self,
        src: &[u8],
        jsstrings_to_escape: &mut [bun_core::String],
        jsobjs_len: u32,
...
pub(crate) fn create_shell_interpreter(
    ...
    if global.has_exception() {
        // Drops root_io and closes `root_shell.cwd_fd`.
        interpreter.deinit_from_exec();
        return Err(crate::jsc::JsError::Thrown);
    }
