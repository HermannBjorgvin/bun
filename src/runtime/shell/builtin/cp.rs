...
impl ShellCpTask {
    ...
    /// when it completes (success or first error). Records the error (if any)
    /// and finishes in place so the interpreter can drain `verbose_output` /
    /// surface it.
    pub(crate) fn cp_on_finish(mut self: Box<Self>, result: bun_sys::Maybe<()>) {
        if let Err(e) = result {
