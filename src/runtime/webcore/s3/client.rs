...
fn new_upload(
    ...
    upload.lifecycle.set(Some(upload.dupe_ref()));
    upload
        .poll_ref
...
pub(crate) fn writable_stream(
    ...
    observer
        .observer
        .set(Some(UploadObserver::Writer(sink.dupe_ref())));
        ...
        match &result {
            S3UploadResult::Success => {
                ...
                if this.end_promise.get().has_value() {
                    ...
                    if settled.is_ok() {
                        settled = end_promise.resolve(&global, JSValue::js_number(0.0));
                    }
                }
            }
            S3UploadResult::Failure(err) => {
...
pub(crate) fn upload_stream(
    ...
    let sink = NetworkSink::new(upload.dupe_ref(), global_this, part_size as BlobSizeType);
    ...
    ctx.pump_ref.set(Some(wrapper.dupe_ref()));
    ...
    if let Some(byte_stream) = readable_stream.ptr.bytes() {
        if byte_stream.sink.get().is_none() {
            ...
            let _ctx_alive = ctx.ref_guard();
            let _sink_alive = sink_ptr.ref_guard();
            sink_ptr
                .source
...
pub(crate) fn readable_stream(
    ...
    let sink = S3DownloadStreamWrapper::new(
        ReadableStreamStrong::init(
            ReadableStream {
                ptr: ReadableStreamPtr::Bytes(context_ptr),
                value: readable_value,
            },
            global_this,
        ),
        path,
        GlobalRef::from(global_this),
    );

    // The task drops the wrapper once the download is over, and the wrapper's
    // `Drop` clears this handle first (holder obligation).
    reader
        .producer
