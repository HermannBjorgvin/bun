...
impl UploadObserver {
    fn dupe_ref(&self) -> Self {
        match self {
            UploadObserver::Writer(sink) => UploadObserver::Writer(sink.dupe_ref()),
            UploadObserver::Stream(wrapper) => UploadObserver::Stream(wrapper.dupe_ref()),
        }
    }

    fn deref(self) {
        match self {
            UploadObserver::Writer(sink) => sink.deref(),
            UploadObserver::Stream(wrapper) => wrapper.deref(),
        }
    }
}
...
impl MultiPartUpload {
    ...
    fn emit_writable(&self, flushed: u64) {
        let Some(observer) = self.observer.get().as_ref().map(UploadObserver::dupe_ref) else {
            return;
        };
        match &observer {
...
impl MultiPartUpload {
    ...
    pub(crate) fn write_encoded(
        ...
        let _deref_guard = self.this_ptr().ref_guard();
