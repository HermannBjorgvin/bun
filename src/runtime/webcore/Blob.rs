...
impl BlobExt for Blob {
    ...
    fn on_structured_clone_deserialize(
        ...
        match on_structured_clone_deserialize(global_this, reader) {
            ...
            Err(_) => {
                Err(global_this.throw(format_args!("Blob.onStructuredCloneDeserialize failed")))
            }
        }
    }
    fn from_url_search_params(
    ...
    fn get_writer(&self, global_this: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> {
        ...
        #[cfg(windows)]
        {
            ...
            // `FileSink::to_js` takes its own per-wrapper ref; release init's.
            let js = sink.to_js(global_this);
            sink.deref();
            return Ok(js);
        }
        ...
        match bytes {
            ...
            SourceBytes::Temporary(owned) => {
                ...
                if buf_range.start != 0 {
                    // +1 WTF ref; `OwnedString` releases it on scope exit.
                    let out = OwnedString::new(BunString::clone_latin1(&owned[buf_range]));
                    drop(owned);
                    return out.to_js(global);
                }
...
pub fn blob_get_file_name_string(this: &crate::webcore::Blob) -> bun_core::String {
