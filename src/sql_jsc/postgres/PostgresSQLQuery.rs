...
pub struct PostgresSQLQuery {
    ...
    // (connection request queue, JS wrapper) / `ScopedRef` (re-entrant paths).
    ref_count: Cell<u32>,
...
// On drop (last ref released): release the statement (if any), then deref
// query/cursor_name. `BunString` is `Copy` (FFI by-value, NO `Drop`), so the
...
impl PostgresSQLQuery {
    ...
    pub(crate) fn release_statement(&self) {
        if let Some(stmt) = self.statement.replace(None) {
            stmt.deref();
        }
    }
        ...
        'enqueue: {
                ...
                .contains(ConnectionFlags::USE_UNNAMED_PREPARED_STATEMENTS)
            {
                ...
                let existing_stmt = connection
                    ...
                    .get()
                    .get(&signature.name[..])
                    .and_then(|slot| slot.as_ref().map(RefPtr::dupe_ref));
