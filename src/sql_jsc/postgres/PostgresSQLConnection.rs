...
pub struct PostgresSQLConnection {
    ...
    // Intrusive refcount (`CellRefCounted`); refs are held as `RefPtr` /
    // `ScopedRef`, and the last release runs `Drop`.
    ref_count: Cell<u32>,
...
impl Drop for PostgresSQLConnection {
    fn drop(&mut self) {
        ...
        self.disconnect();
        self.stop_timers();
        for stmt in self.statements.get_mut_unique().values().flatten() {
